...
// Nota: El proxy (edge) no puede verificar JWT con jsonwebtoken.
...
export default function proxy(request: NextRequest) {
  ...
  // Rutas estáticas y API - no requieren autenticación del proxy
  ...
  if (first && !excluded.includes(first)) {
    ...
    if (!hasAuthToken && !hasRefreshToken && !hasSession) {
      return NextResponse.redirect(new URL(`/${first}/login`, request.url))
    }
    
    // Si hay refresh token pero no access token, intentar auto-refresh en el cliente
    if (!hasAuthToken && hasRefreshToken) {
      // Permitir continuar - el auto-refresh se manejará en el cliente
      edgeLogger.debug('Access token missing but refresh token present, allowing client-side refresh', { pathname })
      return NextResponse.next()
    }
...
export const config = {
  matcher: [
     ...
     * - api routes
     * - _next static files
     * - static file extensions (handled in proxy logic)
     */
    '/((?!api|_next).*)',
  ],
}