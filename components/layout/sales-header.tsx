...
// Importar funciones de cookies centralizadas
...
export function SalesHeader() {
      ...
      if (response.ok) {
        ...
        if (userData) {
          const newSession = {
            ...
            fullName: `${userData.nombre || ''} ${userData.apellido || ''}`.trim() || userData.email || 'Usuario',
  ...
  const fullName = session?.fullName || `${session?.nombre || ''} ${session?.apellido || ''}`.trim() || "Users"
  ...
  useEffect(() => {
    ...
    // Usar color por defecto temporalmente
    if (typeof document !== 'undefined') {
      document.documentElement.setAttribute('data-sas-color', 'green')
    }
  }, [setTheme, slug]) // Incluir dependencias requeridas
          ...
          <div className="flex items-center gap-2">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56">
                ...
                <DropdownMenuItem onClick={() => { const slug = session?.customerSlug || pathname.split("/").filter(Boolean)[0]; window.location.href = `/${slug}/perfil` }}>{"Profile"}</DropdownMenuItem>
                <DropdownMenuSeparator />
              ...
              <div className="absolute right-0 top-full mt-2 w-32 bg-white dark:bg-[#1a1a1a] border border-gray-200 dark:border-[#2a2a2a] rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
                <div className="p-1">
                  ...
                  <button onClick={() => applyTheme('system')} className={`w-full text-left px-3 py-2 rounded hover:bg-gray-100 dark:hover:bg-[#2a2a2a] text-sm ${theme === 'system' ? 'font-semibold' : ''}`}>
                    <Monitor className="inline h-4 w-4 mr-2" /> {"System"}
                  </button>
                </div>
              </div>
            </div>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger className="flex items-center gap-3 outline-none">
              <div className="hidden sm:flex flex-col items-end mr-1">
                ...
                <span className="text-xs text-gray-500 dark:text-gray-400">{session?.rol || "Users"}</span>
              </div>
